...
class AppointmentViewSet(viewsets.ModelViewSet):
    ...
    # 'appointment_date' is a DateTimeField, so an exact match is useless for
    # "show me a day's schedule" — expose __date/__gte/__lte as well so the
    # UI can filter by a calendar date or a range.
    filterset_fields = {
        'doctor':           ['exact'],
        'patient':          ['exact'],
        'status':           ['exact'],
        'appointment_date': ['exact', 'date', 'gte', 'lte'],
    }
